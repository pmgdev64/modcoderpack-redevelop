...
class Commands(object):
    ...
    def readconf(self, workdir, json):
        ...
        if self.jsonFile is None or not os.path.exists(self.jsonFile):
            self.jsonFile = MinecraftDiscovery.getJSONFilename(mcDir, self.versionClient)
        ...
        jarslwjgl = []
        # Toggled: Chế độ nạp thư viện LWJGL 3 mở rộng
        is_modern = 'lwjgl_util' not in self.mcLibraries
        
        if is_modern:
            logging.info("Sử dụng cơ chế quét module cho LWJGL 3 & Addons...")
            # Quét tất cả library từ JSON có chứa từ khóa mục tiêu
            targets = ['lwjgl', 'assimp', 'jemalloc', 'glfw', 'openal', 'stb']
            for lib_id, lib_data in self.mcLibraries.items():
                if any(t in lib_id.lower() for t in targets):
                    lib_file = os.path.join(self.dirjars, lib_data['filename'])
                    if os.path.exists(lib_file):
                        jarslwjgl.append(lib_file)
            
            # AutoSave: Đánh dấu phiên bản Engine để tránh xung đột Recompile
            if self.config.get('DEFAULT', 'engine_type', fallback='') != 'lwjgl3':
                self.config.set('DEFAULT', 'engine_type', 'lwjgl3')
                with open(os.path.normpath(self._default_config), 'w', encoding='utf-8') as f:
                    self.config.write(f)
        else:
            # Giữ tương thích cho bản cũ 1.12
            for key in ['jinput', 'lwjgl', 'lwjgl_util']:
                if key in self.mcLibraries:
                    jarslwjgl.append(os.path.normpath(os.path.join(self.dirjars, self.mcLibraries[key]['filename'])))

        self.jarslwjgl = jarslwjgl
    ...
    def runcmd(self, forkcmd, quiet=False, check_return=True, log_file=None):
        # Toggled: Tự động cấp RAM cho Java 1.13+
        if 'fernflower.jar' in forkcmd and '-Xmx' not in forkcmd:
            forkcmd = forkcmd.replace('java -jar', 'java -Xmx2G -jar')
            
        forklist = cmdsplit(forkcmd)
        is_noisy = any(tool in forkcmd for tool in ['fernflower.jar', 'mcinjector.jar'])

        if not quiet:
            self.logger.debug("runcmd: '%s'", truncate(forkcmd, 500))
        
        if is_noisy:
            # Chạy tool ồn ào nhưng vẫn bắt lỗi nếu return code != 0
            process = subprocess.Popen(forklist, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=-1)
            process.wait()
            # Nếu chạy xong mà không lỗi, giải phóng bộ nhớ bằng cách không đọc output
            if process.returncode == 0:
                output = b"Log suppressed for RAM optimization."
            else:
                output, _ = process.communicate() 
        else:
            # Lệnh bình thường (copy, move...)
            process = subprocess.Popen(forklist, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=-1)
            output, _ = process.communicate()

        # Toggled AutoSave: Ghi log nếu không phải tool "ồn ào"
        if log_file is not None and not is_noisy:
            with open(log_file, 'wb') as log:
                log.write(output)

        # In debug log nếu cần thiết (hoặc khi tool bị crash)
        if not quiet and (not is_noisy or process.returncode != 0):
            for line in output.splitlines():
                try:
                    self.logger.debug(line.decode('utf-8', 'ignore'))
                except:
                    pass
    ...
    def loadsrg(self, srg_file, reverse=False):
        ...
        for line in lines:
            ...
            if type == 'PK:' or type == 'CL:' or type == 'FD:':
                ...
                continue #raise ValueError(f'Unknown type {line}')
