#!/usr/bin/env python 3.13
'''
Edited on jan 18, 2026
...
def copyLibrary(src, dst, name, library):
    try:
        ...
        print(f"Error copying library {name}")
        sys.exit()
...
def extractNative(root, name, jarname, version):
    try:
        ...
        # Đổi zipFile thành zipfile (viết thường toàn bộ)
        with zipfile.ZipFile(srcPath, "r") as jarFile:
            jarFile.extract(name, dstPath)
    except Exception as e:
        # Toggled: Thông báo và bỏ qua nếu file bị lock hoặc lỗi đường dẫn
        print(f"AutoSave Warning: Skipped {name} (Reason: {e})")
        pass
...
def copyMinecraft(src, dst, version):
    try:
        ...
        print(f"Error while copying Minecraft : {e}")
        sys.exit()    
...
def copyClientAssets(commands, workDir = None):
    ...
    for name, library in mcLibraries.items():
        if not MinecraftDiscovery.checkLibraryExists(dstDir, library):
            print(f"Copying library... {name}"),
    ...
    for native, jarname in MinecraftDiscovery.getNatives(dstDir, mcLibraries).items():
        if not MinecraftDiscovery.checkNativeExists(dstDir, native, currentVersion):
            print(f"Extracting native...{native}"),
