...
def getMinecraftPath():
    if   sys.platform.startswith('linux'):
        ...
        print(f"Cannot detect of version : %s. Please report to your closest sysadmin {sys.platform}")
        sys.exit()
...
def getJSONFilename(mcDir, version):
    # Đường dẫn chuẩn: .minecraft/versions/1.13/1.13.json
    json_path = os.path.join(mcDir, 'versions', version, f"{version}.json")
    
    if not os.path.exists(json_path):
        # AutoSave/Log: Nếu không thấy, thử tìm trong thư mục hiện tại của MCP (nếu bạn copy vào jars)
        print(f"  > cannot found on 'Appdata' folder, checking jars/...")
        json_path = os.path.join('jars', 'versions', version, f"{version}.json")
...
def getLibraries(root, jsonfile, osKeyword):
    ...
    try:
        ...
        print(f"Error while parsing the library JSON file : {e}")
        sys.exit()
...
if __name__ == '__main__':
    ...
    for native in mcNatives.keys():
        if checkNativeExists("./jars", native, "1.6.1"):
            print(f'Found {native, mcNatives[native]}')
        else:
            print(f'Not found {native, mcNatives[native]}')
